...
# Activate/deactivate constraints
    ...
    if solver is None:
        ...
        print >>dat, param_n_tours
        print >>dat, param_Shift  
        print >>dat, param_TourType
        print >>dat, param_DailyTourType
        print >>dat, param_DailyShiftWorked
        print >>dat, param_WeekendDaysWorked
        print >>dat, param_tour_WIN_TT
    
        f2_dat = open(phase2_dat_file,'a')
        print >>f2_dat, dat.getvalue()
        f2_dat.close()
        ...
        try:
            ...
            tot_vars = 0
            print "\n\nConstraint summary \n------------------"
            for con_name in phase2_inst.active_components(Constraint):
                con = getattr(phase2_inst,con_name)
                print con_name + " --> " + str(len(con))
                tot_cons += len(con)
            print "\n\nVariable summary \n------------------"
            for var_name in phase2_inst.active_components(Var):
                var = getattr(phase2_inst,var_name)
                print var_name + " --> " + str(len(var))
                tot_vars += len(var)
        ...
        if str(phase2_results.Solution.Status) != 'unknown':
            ...
            phase2_inst.load(phase2_results)  # Put results in model instance
